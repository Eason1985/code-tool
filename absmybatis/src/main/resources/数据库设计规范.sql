 ...
 * 1：设计数据库时必须严格规范。创建一个表后，NAME必须是中文名。COMMENT必须是中文名，COMMENT表示注释。
 注释格式可以参见如下格式（如果使用了这个格式，那么自动生成页面的时候，就可以优化相应的页面）
 	...
 	B : 如果A=0: B的格式为：key-value,key-value
 		如：开户状态：     COMMENT为：     开户状态[0][0-注销,1-正常]
 		
 	    如果A=1: B的格式为：描述值类型:格式化值 
 	    日期：		COMMENT可以为：     日期[1][YYYYMMDD]
 	    
 	    如果A=2: B的格式为：本属性值来源表对应的实体名:id属性(首字母大写)-name属性(首字母大写)
 	    如：网点号：    COMMENT可以为：     网点号[2][NetinfoDO:Net_no-Net_name] NetinfoDO为实体名,Net_no与Net_name分别为首字母大写的实体属性名
