...
public class DatabaseIntrospector {
	...
	private static java.util.Collection<Column> getColumns(DatabaseMetaData databaseMetaData, Table table,String[] excludefields)
		...
		while (rs.next()) {
			...
			column.setRemarks(StringUtil.isNotNullAndBlank(comments)? StringUtil.clearMistyChars(comments, "") : (remarks==null?rs.getString(Column.COLUMN_NAME):remarks));
			
			column.setRemarksMeans(new Column.RemarksMeans(column.getRemarks(),column));
			//排除系统默认字段
