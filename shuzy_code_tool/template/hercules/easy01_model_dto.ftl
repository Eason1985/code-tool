 	...
 	public static class ForeignKeyPropertyEnum{
 		...
 		<#list table.columns as c >	
			<#if   c.remarksMeans.meanType?? && c.remarksMeans.meanType = "2">
			   foreignKeyPropertyEnumList.add(new ForeignKeyProperty("${c.columnName?lower_case}",${c.remarksMeans.tablelName}.class,"${(c.remarksMeans.meanName[0])[0]?lower_case}","${(c.remarksMeans.meanName[0])[1]?lower_case}") );
			</#if>
		</#list>
 		}
