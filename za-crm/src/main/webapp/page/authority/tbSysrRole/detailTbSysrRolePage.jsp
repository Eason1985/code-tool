				...
				<div class="search-panel-bd">
					<table class="search-table">
					   		<tr>
								...
								<td>
									<input type="text" id="detail_role_name" name="detail_role_name"   style="width:200px;border-width: 0" readonly="true" />
								</td>
								<th class="wd-20"><label>角色描述</label></th>
								<td>
									<input type="text" id="detail_description" name="detail_description"   style="width:200px;border-width: 0" readonly="true" />
								</td>
							  </tr>
							 <tr>
								<th class="wd-20"><label>角色组</label></th>
								<td>
									<input type="text" id="detail_role_group" name="detail_role_group"   style="width:200px;border-width: 0" readonly="true" />
								</td>
...
$(document).ready(function(){ 
	...
	acjAjax({
		type:'POST',
		url:'<%=path%>'+"/detail_TbSysrRole.do?pkid="+'<%=pkid%>',    
		success:function(data){
		 	data = $.parseJSON(data);
		 	var message = data.message;
				$("#detail_sysr_role_id").val(message.sysr_role_id); 
				$("#detail_role_name").val(message.role_name); 
				$("#detail_description").val(message.description); 
				$("#detail_role_group").val(message.role_group); 
